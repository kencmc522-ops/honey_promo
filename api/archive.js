...
export default async function handler(req, res) {
  ...
  // GET without cron = show gallery
  if (req.method === 'GET' && !isCron) {
    return res.status(200).json({gallery: global._gallery || [], tip: "Add ?cron=1 to trigger permanent link generation"});
  }
  ...
  try {
    ...
    const svg = `<svg width="1080" height="1080" xmlns="http://www.w3.org/2000/svg"><rect width="1080" height="1080" fill="#fdf6e3"/><text x="60" y="80" font-family="sans-serif" font-size="32" fill="#8d6e63">${today}</text><rect x="60" y="140" width="600" height="600" rx="40" fill="#ffffff"/><text x="360" y="520" text-anchor="middle" font-size="200">🧇</text><text x="720" y="280" font-family="sans-serif" font-size="96" font-weight="900" fill="#3e2723">${item.name}</text><text x="720" y="380" font-family="sans-serif" font-size="72" fill="#bf360c" font-weight="700">${item.price}</text><text x="720" y="450" font-family="sans-serif" font-size="36" fill="#5d4037">${item.desc}</text><text x="720" y="550" font-family="sans-serif" font-size="28" fill="#8d6e63">每日新鮮出爐</text><text x="60" y="1010" font-family="sans-serif" font-size="28" fill="#a1887f">@eggette.daily</text></svg>`;
    ...
    return res.status(200).json({ success:true, cron:isCron, permanentUrl, ig: igResult, gallery: global._gallery });
  } catch (e) {
